# Type module: budget-tiers

## When this applies
`articleType` in the brief is `"budget-tiers"`. Products span a wide price range (lowest to highest is ≥2×). The article organizes products by budget bracket rather than by product-first ranking.

## Target word count
- **Total**: 800-1300 words
- No rigid per-product section minimum, but each tier must be substantive
...
2. **Budget bracket sections** (H2 per bracket)
   ...
   - Recommend the best product in the bracket (affiliate link on name)
   - 100-200 words explaining WHY it wins at that budget (concrete specs from brief)
   - Image placement at start, widget at end of each bracket section
   - One-sentence runner-up mention if multiple products fit the bracket

3. **Verdict** (H2: "Vores dom", 80-120 words)
   - "Bedste valg for pengene" recommendation
   - Decision shortcut: "Har du X kr. at bruge, er [produkt] det klare valg."
